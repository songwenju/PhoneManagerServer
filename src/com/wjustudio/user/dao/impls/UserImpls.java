...
public class UserImpls extends BaseDB implements UserDao{
		...
		if(LoginIdentity.matches("\\w+([-+.]\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*")){
			try {
				if(super.res.next()){
					...
					user.setHeadImgUrl(super.res.getString("headImgUrl"));
				}
	...
	public int updateUserImgUrl(String name, String imgUrl) {
		int flag = 0;
		String sql =  "update "+USER_TABLE+" set headImgUrl = '"
				+imgUrl +"' where userName = '" +name+"'";
		System.out.println(sql);
		try{
			super.executeUpdate(sql);
			flag = 1;
	...
	public int getUserId(String name) {
		...
		super.res = super.executeQuery(sql, name);
		try {
			if (super.res.next()) {
				
				return  super.res.getInt("id");
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
