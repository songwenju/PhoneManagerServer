...
public class MD5_Encoding {
	...
	//最新一次计算结果是16进制ASCII字符串表示，代表了16个字符串形式的MD5值
	public String resultStr;

	// 最新一次计算结果是2进制数组表示，一共16个字节，代表了128bit形式的MD5值
	public byte[] digest = new byte[16];

	// MD5_Encoding类提供的主要接口函数getMD5ofStr,用来进行数据加密转换。
	...
	public String getMD5ofStr(String in) {
		...
		for (int i = 0; i < 16; i++) {
			resultStr += byteToHEX(digest[i]); // 将digest数组中的每个byte型数据转换为Ϊ16进制形式的字符串
		}
		return resultStr;
	}
	...
	private void md5Init() {
		state[0] = 0x67452301L;  // 定义state为RFC1321中定义标准幻数
		state[1] = 0xefcdab89L;  // 定义state为RFC1321中定义标准幻数
		state[2] = 0x98badcfeL;  // 定义state为RFC1321中定义标准幻数
		state[3] = 0x10325476L;  // 定义state为RFC1321中定义标准幻数
	...
	//定义F G H I 即为4个基本的MD5函数，进行简单的位运算
	...
	private long FF(long a, long b, long c, long d, long x, long s, long ac) {
		a += F(b, c, d) + x + ac;
		a = ((int) a << s) | ((int) a >>> (32 - s));  //这里的long型数据右移时使用无符号优异运算符>>>
		a += b;
		return a;
	}
	...
	// MD5的主要计算过程，input是需要变换的二进制字节串，inputlen是长度
	private void md5Update(byte[] input, int inputLen) {
		int i=0, index, partLen;
		byte[] block = new byte[64];   // 分配64个字节缓冲区
		//根据count计算index这里的long型数据右移时使用无符号优异运算>>>
		index = (int) (count[0] >>> 3) & 0x3F; 
		if ((count[0] += (inputLen << 3)) < (inputLen << 3))
			count[1]++;
		count[1] += (inputLen >>> 29);  //这里的int型数据右移时使用无符号优异运算符>>>
